...
| `src/cli.ts`, `src/index.ts` | — | Composition. The only modules that may import all three. |
...
| Session capture and expiry | `gap/curl.ts`, `gap/session.ts` | Capture content is a credential and is never persisted, logged, or exposed. |
