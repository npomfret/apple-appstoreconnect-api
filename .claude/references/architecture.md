...
| Authenticated requests and uploads | `http.ts` | All ordinary writes are audited; paths are relative to Iris and an absolute URL is refused, so session headers reach no other host; upload URLs never receive session headers. |
