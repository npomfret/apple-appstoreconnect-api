...
the private Iris or Xcode Cloud transport. The official operations are storefront
availability and a TestFlight group's builds — the latter carrying the first official
writes, on 2026-09-02, chosen for being reversible and each other's undo. Almost everything
on the private side is about an app or one of its Xcode Cloud products. The
...
| --- | --- | --- |
| Official API credentials and transport | `official/client.ts` | Issuer, key and key path are runtime inputs with no account-specific defaults. JWTs go only to `api.appstoreconnect.apple.com`. The interface is `get()` and `write()`, the latter taking a `WriteMethod` of `POST`, `PATCH` or `DELETE` and never `GET`, so a read cannot pass through the audited path and a write cannot avoid it. Every official mutation leaves through `write()`, which emits the `official.http.write` audit record whatever the log level, after the URL check and before the request. Tokens are minted on demand and re-minted before expiry, so lifetime never bounds a run. |
| Official response narrowing | `official/parse.ts` | The `unknown` off the transport becomes a typed value one field at a time, and a field of the wrong shape is refused by name rather than defaulted. Shared by every wrapper so the rule exists once; not exported from the package. |
| Which app an official command is about | `official/apps.ts` | An app id as given, or one resolved from a bundle ID or from the exact name App Store Connect shows, through the documented filters and matched exactly on the rows that come back. None or several is an error naming what Apple offered; nothing is guessed and no identifier lives in configuration. Every official command takes the app the same three ways, decided once in `cli.ts`. |
| Official storefront availability | `official/availability.ts` | All territory rows are parsed, a paged response is refused rather than under-reported, and storefront state comes from the enumerated 4.4.1 `contentStatuses` union — worst status wins, unrecognised values are reported as unknown rather than bucketed, and Apple's strings are kept verbatim. |
| Official TestFlight group builds | `official/testflight.ts` | Both directions of one documented relationship, `betaGroups/{id}/relationships/builds`, and nothing that expires or deletes a build. The plan is a read and the write takes the plan, so what is sent is exactly what was shown: ids, never a recomputed set. A group is matched by exact name or by id within an app and refused when none or several match; A group carrying Apple's `hasAccessToAllBuilds` flag is refused before any build is listed, because the one live `DELETE` sent to such a group was answered `204` and removed nothing; the flag is not settable through the official API. A build is named by number or by id, told apart by uuid shape, and must resolve to exactly one. Newest is by the instant Apple stamped, not the text, and `keep` counts per platform, since a group holds iOS and macOS builds together and a tester installs only one kind. One page of 200 per read; a further page is reported, never followed, and keeps `--check` red. Every write is followed by a read of the group, and the result carries what the write did not account for. Nothing is sent for an empty plan. |
...
- `request()` is the audit choke point for private mutations, and `OfficialClient.write()`
  for official ones. Do not bypass either; they cannot be one function, since each holds a
  credential the other may not.
