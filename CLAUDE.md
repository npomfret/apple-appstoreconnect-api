...
- `src/shared/` — what neither credential touches: logging and redaction, the query
  ...
  may live in it.
- `src/cli.ts` and `src/index.ts` compose all three; nothing else does.
...
- `src/gap/http.ts`, `src/gap/session.ts`, `src/gap/curl.ts`, `src/official/client.ts`,
  `src/shared/log.ts`, `src/shared/errors.ts`, and `src/shared/confirm.ts` are security
  boundaries. Follow their scoped rules and require evidence for changes.
